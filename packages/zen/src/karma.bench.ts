...
import { karma, runKarma } from './karma'; // Import functional API
...
describe('Task Run (Reject)', () => {
  ...
  const zTaskReject = karma(asyncFnReject);

  bench('zen (reject)', async () => {
    try {
      await runKarma(zTaskReject);
    } catch {
