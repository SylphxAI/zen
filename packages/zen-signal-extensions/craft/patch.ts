...
function _applyModifyPatch<T>(patch: Patch, baseState: T, draft: T): void {
  ...
  setValueByPath(
    ...
    patch.path,
    patch.value,
    patch.op as 'add' | 'replace' | 'remove',
  );
}
...
function _applyCopyPatch<T>(patch: Patch, baseState: T, draft: T): void {
  ...
  if (valueToCopy === undefined && getValueByPath(baseState, patch.from) === undefined) {
    throw new Error(
      `'copy' operation source path does not exist or is invalid: ${patch.from.join('/')}`,
    );
  }
...
function _handleRootOperation(patch: Patch): void {
  if (patch.op === 'replace' || patch.op === 'add') {
    throw new Error(
      'Root replacement/addition via applyPatches is not supported. Use produce directly.',
    );
  }
  if (patch.op === 'remove') {
