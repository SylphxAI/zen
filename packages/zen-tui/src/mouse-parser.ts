...
export interface MouseEvent {
  type: 'click' | 'scroll';
  button: 'left' | 'middle' | 'right' | 'scroll-up' | 'scroll-down';
...
export function parseMouseEvent(data: string): MouseEvent | null {
  ...
  // Only fire click events on mouse up (action === 'm')
  // Scroll events fire on 'M'
  if (type === 'click' && action === 'M') {
    return null; // Ignore mouse down for clicks
  }
