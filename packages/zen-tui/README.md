...
**@zen/tui** is a modern, high-performance terminal UI library that combines the simplicity of [React Ink](https://github.com/vadimdemedes/ink) with the power of fine-grained reactivity.

## Features

✨ **React Ink Compatible** - Drop-in replacement for most Ink components
⚡ **Fine-Grained Reactivity** - Powered by `@zen/signal` for precise updates
...
```typescript
import { terminalWidth } from '@zen/tui';
...
```tsx
import { MouseProvider, Pressable, Box, Text } from '@zen/tui';
...
```tsx
import { ScrollBox, Text } from '@zen/tui';
...
- **Zen Signal**: [@zen/signal](../zen-signal)
