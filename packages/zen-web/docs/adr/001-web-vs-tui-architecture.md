...
## Context
The Zen framework provides two rendering targets: `@zen/web` for browser DOM and `@zen/tui` for terminal UI. While they share the same reactive runtime (`@zen/runtime`, `@zen/signal`), their rendering approaches differ significantly due to platform constraints.

## Decision

### Shared Architecture
Both renderers share:
- **Reactive primitives**: Signals, effects, computed values from `@zen/signal`
- **Control flow components**: `For`, `Show`, `Switch`, `Match`, `ErrorBoundary`, `Suspense` from `@zen/runtime`
...
- Code sharing via `@zen/runtime` components
